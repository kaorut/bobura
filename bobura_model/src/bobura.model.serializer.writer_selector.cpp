...
namespace bobura { namespace model { namespace serializer
{
    ...
    public:
        ...
        bool selects_impl(const boost::filesystem::path& path)
        const
        {
            return
                std::find_if(
...
#   pragma GCC diagnostic warning "-Wreturn-type"
    ...
    :
    base_type(),
    m_p_impl(tetengo2::stdalt::make_unique<impl>(std::move(p_writers), std::move(path)))
    {}
    ...
    writer_selector<Size, Difference, String, OutputStream, OperatingDistance, Speed, Font>::extension_impl()
    const
    {
