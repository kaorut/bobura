...
#include <bobura/command/file_property.h> // IWYU pragma: keep
#include <bobura/command/font_color.h> // IWYU pragma: keep
#include <bobura/command/horizontally_zoom_in.h> // IWYU pragma: keep
#include <bobura/command/horizontally_zoom_out.h> // IWYU pragma: keep
#include <bobura/command/load_from_file.h> // IWYU pragma: keep
#include <bobura/command/new_file.h> // IWYU pragma: keep
#include <bobura/command/nop.h> // IWYU pragma: keep
#include <bobura/command/open_www_tetengo_org.h> // IWYU pragma: keep
#include <bobura/command/save_to_file.h> // IWYU pragma: keep
#include <bobura/command/set.h>
#include <bobura/command/set_horizontal_scale.h> // IWYU pragma: keep
#include <bobura/command/set_vertical_scale.h> // IWYU pragma: keep
#include <bobura/command/show_diagram.h> // IWYU pragma: keep
#include <bobura/command/show_timetable_down.h> // IWYU pragma: keep
#include <bobura/command/show_timetable_up.h> // IWYU pragma: keep
#include <bobura/command/train_kind.h> // IWYU pragma: keep
#include <bobura/command/vertically_zoom_in.h> // IWYU pragma: keep
#include <bobura/command/vertically_zoom_out.h> // IWYU pragma: keep
...
namespace bobura { namespace command {
    ...
    public:
        ...
        {
            return tetengo2::stdalt::make_unique<
        ...
        const command_type& set_horizontal_scale(const size_type index) const
        {
            return *m_p_set_horizontal_scale[index];
        }

        const command_type& set_vertical_scale(const size_type index) const
        {
    ...
    private:
        ...
        static command_ptr_type create_about(
            ...
            const detail_impl_set_type& detail_impl_set)
        {
            return tetengo2::stdalt::make_unique<command::about<
                command_traits_type,
                position_type,
                dimension_type,
        ...
        {
            return tetengo2::stdalt::make_unique<command::font_color<
                command_traits_type,
                dialog_type,
                ...
                color_dialog_type,
                message_catalog_type,
                dialog_traits_type>>(message_catalog);
        }

        static command_ptr_type create_horizontally_zoom_in(diagram_view_type& diagram_view)
        {
            return tetengo2::stdalt::make_unique<command::horizontally_zoom_in<
                command_traits_type,
                traits_type,
                main_window_traits_type,
                diagram_view_traits_type>>(diagram_view);
        }

        static command_ptr_type create_horizontally_zoom_out(diagram_view_type& diagram_view)
        {
            return tetengo2::stdalt::make_unique<command::horizontally_zoom_out<
                command_traits_type,
                traits_type,
                main_window_traits_type,
                diagram_view_traits_type>>(diagram_view);
        ...
        {
            return tetengo2::stdalt::make_unique<command::load_from_file<command_traits_type, load_save_traits_type>>(
                load_from_file);
        }

        static command_ptr_type create_new_file(const new_file_type& new_file)
        {
            return tetengo2::stdalt::make_unique<command::new_file<command_traits_type, load_save_traits_type>>(
                new_file);
        }
        ...
        {
            return tetengo2::stdalt::make_unique<command::save_to_file<command_traits_type, load_save_traits_type>>(
                save_to_file);
        }
        ...
        static std::vector<command_ptr_type> create_set_horizontal_scale(diagram_view_type& diagram_view)
        {
            ...
            {
                commands.push_back(tetengo2::stdalt::make_unique<command::set_horizontal_scale<
                                       command_traits_type,
                                       scale_type,
                                       traits_type,
                                       main_window_traits_type,
                                       diagram_view_traits_type>>(diagram_view, scale_list.at(i)));
            }
        ...
        static std::vector<command_ptr_type> create_set_vertical_scale(diagram_view_type& diagram_view)
        {
            ...
            for (size_type i = 0; i < scale_list.size(); ++i)
            {
                commands.push_back(tetengo2::stdalt::make_unique<command::set_vertical_scale<
                                       command_traits_type,
                                       scale_type,
                                       traits_type,
                                       main_window_traits_type,
        ...
        {
            return tetengo2::stdalt::make_unique<command::train_kind<
                command_traits_type,
                dialog_type,
                point_dimension_unit_type,
                color_type,
        ...
        {
            return tetengo2::stdalt::make_unique<command::vertically_zoom_in<
                command_traits_type,
                traits_type,
                main_window_traits_type,
                diagram_view_traits_type>>(diagram_view);
        ...
        {
            return tetengo2::stdalt::make_unique<command::vertically_zoom_out<
                command_traits_type,
                traits_type,
                main_window_traits_type,
                diagram_view_traits_type>>(diagram_view);
    ...
    template <typename Traits>
    set<Traits>::~set() noexcept
    {}

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::about() const
    {
        return m_p_impl->about();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::ask_file_path_and_save_to_file() const
    {
        return m_p_impl->ask_file_path_and_save_to_file();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::exit() const
    {
        return m_p_impl->exit();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::file_property() const
    {
        return m_p_impl->file_property();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::font_color() const
    {
        return m_p_impl->font_color();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::horizontally_zoom_in() const
    {
        return m_p_impl->horizontally_zoom_in();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::horizontally_zoom_out() const
    {
        return m_p_impl->horizontally_zoom_out();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::load_from_file() const
    {
    ...
    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::new_file() const
    {
        return m_p_impl->new_file();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::nop() const
    {
        return m_p_impl->nop();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::open_www_tetengo_org() const
    {
        return m_p_impl->open_www_tetengo_org();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::reload() const
    {
        return m_p_impl->reload();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::save_to_file() const
    {
        return m_p_impl->save_to_file();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::set_horizontal_scale(const size_type index) const
    {
        return m_p_impl->set_horizontal_scale(index);
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::set_vertical_scale(const size_type index) const
    {
        return m_p_impl->set_vertical_scale(index);
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::show_diagram() const
    {
        return m_p_impl->show_diagram();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::show_timetable_down() const
    {
        return m_p_impl->show_timetable_down();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::show_timetable_up() const
    {
        return m_p_impl->show_timetable_up();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::train_kind() const
    {
        return m_p_impl->train_kind();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::vertically_zoom_in() const
    {
        return m_p_impl->vertically_zoom_in();
    }

    template <typename Traits>
    const typename set<Traits>::command_type& set<Traits>::vertically_zoom_out() const
    {
