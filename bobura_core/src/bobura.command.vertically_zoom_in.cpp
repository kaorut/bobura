...
namespace bobura { namespace command
{
    ...
    class vertically_zoom_in<Traits>::impl
    {
    public:
        // types
    ...
    template <typename Traits>
    vertically_zoom_in<Traits>::vertically_zoom_in(diagram_view_type& diagram_view)
    :
    m_p_impl(tetengo2::stdalt::make_unique<impl>(diagram_view))
    {}

    template <typename Traits>
    vertically_zoom_in<Traits>::~vertically_zoom_in()
    TETENGO2_STDALT_NOEXCEPT
    {}
    
    template <typename Traits>
    void vertically_zoom_in<Traits>::execute_impl(model_type& model, abstract_window_type& parent)
    const
    {
