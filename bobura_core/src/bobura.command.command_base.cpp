...
namespace bobura { namespace command {
    ...
    command_base<Traits>::~command_base() noexcept
    {}

    template <typename Traits>
    bool command_base<Traits>::enabled(const model_type& model) const
    {
        return enabled_impl(model);
    }

    template <typename Traits>
    typename command_base<Traits>::state_type command_base<Traits>::state() const
    {
        return state_impl();
    }

    template <typename Traits>
    void command_base<Traits>::execute(model_type& model, abstract_window_type& parent) const
    {
        execute_impl(model, parent);
    }

    template <typename Traits>
    void command_base<Traits>::execute(model_type& model, abstract_window_type& parent, const parameter_base& parameter)
        const
    {
    ...
    template <typename Traits>
    typename command_base<Traits>::state_type command_base<Traits>::state_impl() const
    {
