...
namespace bobura { namespace message {
    namespace diagram_view {
        ...
        namespace detail {
            ...
            {
                return (boost::basic_format<typename String::value_type>(String{ TETENGO2_TEXT("%1.1f") }) %
        ...
        */
        ...
        private:
            ...
            string_type grade_text(const grade_type& grade) const
            {
            ...
            string_type operating_distance_text(const operating_distance_type& operating_distance) const
            {
        ...
        */
        ...
        private:
            ...
            string_type kind_name_text(const train_type& train) const
            {
            ...
            string_type name_text(const train_type& train) const
            {
            ...
            string_type departure_and_arrival(const train_type& train, const size_type stop_index) const
            {
            ...
            string_type departures_and_arrivals(const train_type& train) const
            {
                string_type text{};

                for (auto i = train.origin_stop(); i != train.stops().end(); i = train.next_stop(i))
                {
                    if (i == train.origin_stop() || i == train.destination_stop() ||
            ...
            bool is_principal_station(const stop_iterator i_front_stop, const stop_iterator i_stop) const
            {
                const size_type stop_index = std::distance(i_front_stop, i_stop);
                assert(stop_index < m_model.timetable().station_locations().size());

                return &m_model.timetable().station_locations()[stop_index].get_station().grade() !=
                ...
                const bool          departure) const
            {
                ...
                stream << m_model.timetable().station_locations()[stop_index].get_station().name()
                ...
                {
                    stream << string_type{ TETENGO2_TEXT(" (") }
                           << m_message_catalog.get(TETENGO2_TEXT("PropertyBar:Operational"))
                           << string_type{ TETENGO2_TEXT(")") };
                }

                if (!i_stop->platform().empty())
                    stream << string_type{ TETENGO2_TEXT(" [") } << i_stop->platform()
                           << string_type{ TETENGO2_TEXT("]") };

                return stream.str();
            }

            string_type time_text(const time_type& time, const bool departure) const
            {
                ...
                if (departure)
                {
                    stream << boost::basic_format<typename string_type::value_type>(
            ...
            scheduled_speed_text(const train_type& train, const boost::optional<size_type>& departure_stop_index) const
            {
                ...
                const auto scheduled_speed = m_model.timetable().scheduled_speed(
                    train, departure_and_arrival.first, departure_and_arrival.second);
